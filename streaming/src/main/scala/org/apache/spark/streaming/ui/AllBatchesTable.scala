...
private[ui] abstract class BatchTableBase(tableId: String, batchInterval: Long) {
  ...
  protected def columns: Seq[Node] = {
    <th>Batch Time</th>
      <th>Input Size</th>
      <th>Input Size Limit
        {SparkUIUtils.tooltip("The upper bound of input size of a batch; defined by back pressure",
          "top")}
      </th>
      <th>Scheduling Delay
  ...
  protected def baseRow(batch: BatchUIData): Seq[Node] = {
    ...
    val eventCountLimit = batch.numRecordsLimit
    ...
    <td id={batchTimeId} sorttable_customkey={batchTime.toString}
      ...
      <td sorttable_customkey={eventCountLimit.toString}>{eventCountLimit.toString} events</td>
