 ...
 * @param dataSets the data set for the timeline graph
...
function drawTimeline(id, dataSets, minX, maxX, minY, maxY, unitY, batchInterval) {
    ...
    for (var dataSetIdx = 0; dataSetIdx < dataSets.length; dataSetIdx ++) {
      svg.append("path")
         ...
         // 0th line is the record rate line
         // 1st, 3rd, 5th... lines are the *dashed* rate limit segments
         // 2nd, 4th, 6th... lines are the *solid* rate limit segments
         .attr("class", dataSetIdx == 0 ?
                        "line" :
                        (dataSetIdx % 2 == 1 ? "line rate_limit_line_dashed" :
    ...
    svg.selectAll(".point")
        // Only add points of the 0th line, i.e., the record rate line
        .data(dataSets[0])
        .enter().append("circle")
