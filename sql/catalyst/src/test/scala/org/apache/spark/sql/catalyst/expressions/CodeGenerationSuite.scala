...
class CodeGenerationSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("compilation should respect func size hints") {
    ...
    val funcSizeHints = Map("inc" -> CodegenContext.ERROR_IF_LARGER_THAN_JIT_LIMIT)
    ...
    // compilation should pass against because 1000 loc would be compiled to ~3K bytes
    val code2 = new CodeAndComment(genCode(1000), emptyComments, funcSizeHints)
    CodeGenerator.compile(code2)

    // compilation should fail against because 10000 loc would be compiled to ~30K bytes
    ...
    // compilation should fail against because 15000 loc would be compiled to ~45K bytes
    val ctx = new CodegenContext()
    ctx.addNewFunction("inc", genCode(15000), CodegenContext.ERROR_IF_LARGER_THAN_JIT_LIMIT)
