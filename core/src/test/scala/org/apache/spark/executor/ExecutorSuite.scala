...
import org.apache.spark.{ExceptionFailure, _}
...
class ExecutorSuite extends SparkFunSuite {
  ...
  test("SPARK-15963: Catch `TaskKilledException` correctly in Executor.TaskRunner") {
    ...
    val executor = new Executor("id", "host", mockEnv, userClassPath = Nil, isLocal = true)
    ...
    // `exception` should be TaskKilledException; `taskState` should be `KILLED`
    val exceptionOption = TestHelper.testFailedReason.asInstanceOf[ExceptionFailure].exception
    assert(exceptionOption.get.isInstanceOf[TaskKilledException])
